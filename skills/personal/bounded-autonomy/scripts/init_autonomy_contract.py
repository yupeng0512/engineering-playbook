...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scaffold a .autonomy contract into a target repository."
    )
...
def template_root() -> Path:
    root = playbook_root() / "templates" / "autonomy-contract" / ".autonomy"
    if not root.is_dir():
...
def destination_for(template_file: Path, root: Path, target_repo: Path) -> Path:
    ...
    if name.endswith(".tmpl"):
        name = name[: -len(".tmpl")]
    return target_repo / ".autonomy" / relative_path.with_name(name)
...
def main() -> int:
    ...
    if args.dry_run:
        print(
            f"[dry-run] Would scaffold .autonomy contract for repo '{repo_name}' in {target_repo}"
        )
    else:
        print(
            "Next step: run "
            f"python {Path(__file__).with_name('validate_autonomy_contract.py')} {target_repo}"
        )
    return 0
